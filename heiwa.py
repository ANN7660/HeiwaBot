...
WELCOME_CHANNEL_ID = 1425082379768303649  # ID du salon de bienvenue
LEAVE_CHANNEL_ID = 1425083330251980840    # ID du salon des départs
...
def get_repl_url():
    """Affiche l'URL du Repl automatiquement"""
...
# ===== NOUVELLE COMMANDE POUR AFFICHER L'URL =====
@bot.command(name='url', aliases=['link', 'repl_url'])
@commands.has_permissions(administrator=True)
async def show_repl_url(ctx):
    """🌐 Affiche l'URL du Repl pour UptimeRobot"""
    repl_name = os.getenv('REPL_SLUG')
    repl_owner = os.getenv('REPL_OWNER')
    ...
    if repl_name and repl_owner:
        url = f"https://{repl_name}.{repl_owner}.repl.co"
        embed.description = f"**URL de votre Repl:**\n```{url}```"
        embed.add_field(name="📋 Pour UptimeRobot", 
                       value="Copiez cette URL dans UptimeRobot pour maintenir le bot actif 24h/24", 
                       inline=False)
        embed.add_field(name="🔧 Nom du Repl", value=repl_name, inline=True)
        embed.add_field(name="👤 Propriétaire", value=repl_owner, inline=True)
        embed.add_field(name="⏰ Intervalle recommandé", value="5 minutes", inline=True)
    else:
        embed.description = "❌ Impossible de déterminer l'URL automatiquement"
        embed.add_field(name="💡 Solution", 
                       value="Vérifiez dans l'onglet 'Webview' de Replit\nFormat: `https://nom-du-repl.nom-utilisateur.repl.co`", 
                       inline=False)
...
async def mute_member(ctx, member: discord.Member, duration: int = 10, *, raison="Aucune raison fournie"):
    """🔇 Mute un membre (durée en minutes)"""
    if member == ctx.author:
    ...
    try:
        ...
        if muted_role in member.roles:
            ...
            unmute_embed = discord.Embed(
                ...
                description=f"**{member.display_name}** a été démuté automatiquement",
                color=discord.Color.green()
            )
...
async def unmute_member(ctx, member: discord.Member):
    ...
    muted_role = discord.utils.get(ctx.guild.roles, name="Muted")
    if not muted_role:
        return await ctx.send("❌ Aucun rôle 'Muted' trouvé!")

    if muted_role not in member.roles:
        return await ctx.send("❌ Ce membre n'est pas muté!")

    try:
        await member.remove_roles(muted_role, reason=f"Démuté par {ctx.author}")
...
async def help_moderation(ctx):
    ...
    commands_list = [
        ...
        ("🌐 `!url`", "Affiche l'URL du Repl pour UptimeRobot"),
        ("📩 **Messages privés:**", ""),
...
if __name__ == "__main__":
    ...
    # Démarre le serveur web AVANT le bot
    keep_alive()
    ...
    # Affiche l'URL pour UptimeRobot
    get_repl_url()
    ...
    # Démarre le bot Discord avec gestion complète des erreurs
    try:
